...
class VersionSpaceLearner(learn.BaseLearner):
    ...
    def canidate_elimination(self, examples):
        ...
        h_s = None
        self._logger.debug('h_g: %s #BLUE', h_g)
        self._logger.debug('h_s: %s #YELLOW', h_s)
        ...
        for ex in examples:
            ...
            if ex[1]:
                ...
                self._logger.debug('h_g pruned to: %s #BLUE', h_g)

                # For h_s: Generalize h_s or initilize if first positive example
                if h_s is None:
                    h_s = [ex[0]]
                    self._logger.debug('h_s initilized to: %s #YELLOW', h_s)
                    self._logger.debug('Example Learned. #BOLD')
                    continue
                ...
                for h in h_s_copy:
                    if h is not self.consistent(h, [ex], 'g'):
                        ...
                        if any(self.generality(g, h_new) for g in h_g) and self.consistent(h_new, [ex], 'g'):
                            self._logger.debug('Generalize %s to %s #YELLOW', h, h_new)
                        ...
                        # Prune otherwise
                            ...
                            self._logger.debug(
                                '%s is more general than any h in h_g #YELLOW', h_new)
            ...
            # Negative example
                ...
                if h_s is not None:
                    ...
                    self._logger.debug('h_s pruned to: %s #YELLOW', h_s)
                    ...
                    if not self.consistent(h, [ex], 'b'):
                        ...
                        for i in xrange(len(h)):
                            if h[i] is None:
                                ...
                                # and h_new is consistent with example
                                if (h_s is None or any(self.generality(h_new, s) for s in h_s)) \
                                        and self.consistent(h_new, [ex], 'b'):
                                    self._logger.debug('Specified %s to %s #BLUE', h, h_new)
                                ...
                                # Prune otherwise
                                    ...
                                    self._logger.debug(
                                        '%s is more specific than any h in h_s #BLUE', h_new)

            self._logger.debug('h_g: %s #BLUE', h_g)
            self._logger.debug('h_s: %s #YELLOW', h_s)
            self._logger.debug('Example Learned. #BOLD')

            # Indicate if h_g and h_s converge
            if h_g == h_s:
                self._logger.debug('h_g and h_s converged #BG_GREEN')
    ...
    def consistent(self, h, examples, color):
        ...
        for ex in examples:
            ...
            if not consistent:
                if color == 'b':
                    self._logger.debug('%s is not consistent with %s #BLUE', h, ex)
                else:
                    self._logger.debug('%s is not consistent with %s #YELLOW', h, ex)
                return consistent

        if color == 'b':
            self._logger.debug('%s is consistent with %s #BLUE', h, examples)
        else:
            self._logger.debug('%s is consistent with %s #YELLOW', h, examples)
        return consistent
