...
class PyQtView(view.BaseView):
    def __init__(self, model, ctrl):
        ...
        queue_format = logging.Formatter('%(asctime)s [%(module)s:%(funcName)s():%(lineno)d] [%(levelname)s] %(message)s')
        queue_handler.setFormatter(queue_format)
    ...
    def ImportGUI(self):
        ...
        # Threading
        # self._gui.loggerText.append("Kevin! \n")
    ...
    def update_from_model(self):
        ...
        while not self.queue.empty():
            self._gui.loggerText.append(self.queue.get().message)
